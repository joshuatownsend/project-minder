...
- Cost report: `CostReportDashboard` at `/costs` — all-projects cost ranking for one period (sortable table, six-period switcher `today/7d/30d/90d/1y/all` = `COST_PERIODS`), rows link to each project's `Costs` tab. Per-project `CostsTab` on `ProjectDetail` mirrors it (headline + by-model/by-category). Both reuse `GET /api/usage?period=&project=`. Cross-referencing a route slug to usage aggregates uses `ProjectData.usageSlug` (scanner-derived; the usage module keys turns on the encoded-conversation-dir slug `dev-<name>`, not the filesystem route slug)
...
- Workflow launcher: `LauncherChips` (`src/components/LauncherChips.tsx`) — one-click "run this workflow" chips that dispatch a single task via `POST /api/tasks` (scoped to a project through `metadata.projectPath`). Two placements gated by the default-on `workflowLauncher` flag: a per-project "Quick Launch" strip on `ProjectDetail` (direct dispatch) and `GlobalLauncherRow` under the top bar (project-picker dispatch; hidden on `/project/`). Curated definitions + pure dispatch builders in `src/lib/launchers/definitions.ts` (`LAUNCHER_WORKFLOWS`, `buildWorkflowDispatch`, `buildSkillDispatch`, `selectSkillChips`); skill chips come from `/api/skills` (`userInvocable && !disabled`). Note: `validateCreateTask` threads `metadata` through (it was previously dropped — the spawner's `taskCwd` reads `metadata.projectPath`/`worktreePath`); `POST /api/tasks` carries the shared `demoWriteBlock()` guard (demo mode is read-only, 409). A gentler on-ramp than `SwarmComposer` (`/api/swarms`).
...
- Project Groups: `/group/[slug]` (`GroupDetail` in `src/components/groups/`) — one repo checked out in several scanned locations, aggregated client-side via `aggregateGroup()` from the dashboard's `ScanResult` (no group API). Locations strip, read-only repo-borne tabs with `PresenceChips` divergence flags (pure `src/lib/groups/presence.ts`), Costs summed per `usageKeys` entry (`useGroupUsage` + `sumUsageReports`), Sessions per location, Environments over `/api/environments`. Member cards carry an `N loc` chip; a group of one is never emitted. Plan: `docs/superpowers/plans/2026-07-20-project-groups-multi-location.md`
...
**`INSIGHTS.md` entries belong in the PR whose session produced them.** They are provenance — the record of how the code in that PR came to look the way it does — so include them in the feature commit and let them ride along in the PR. Don't reflexively segregate them into a standalone `chore:` commit; that is the fallback for when there is genuinely no related PR (a session that shipped nothing, or entries orphaned after the fact), not the default. A hook appends them mid-session, so a dirty `INSIGHTS.md` while you work is the system functioning, not noise to suppress.

*Exception — worktree sessions.* Rule 8 under Manual Step Logging sends `INSIGHTS.md` entries to the **canonical main-tree** checkout, and Minder's writers redirect there automatically, so in a worktree the entry lands outside your branch and **cannot** ride along in the PR commit. Don't fight that by writing a worktree-local copy — the canonical rule exists so planning doesn't fragment into per-branch copies invisible until merge. Instead commit the entry in the parent checkout and **name the PR number in that commit message**, which is the one case the standalone `chore:` commit is genuinely for: provenance that is structurally unable to travel with its change.

**Do not git-ignore `INSIGHTS.md`.** Tried and closed unmerged in PR #437, after two rounds of review findings and direct measurement. An ignore rule does not protect an already-tracked file: `git switch` to any branch or tag that still tracks the path **silently overwrites** the working copy with that ref's historical version, and switching back **deletes it entirely** — exit 0, no warning, because `--overwrite-ignore` is checkout's default. The merge-abort that protects uncommitted changes does not apply to checkout. This never expires, since history permanently contains the path.

...
| P1 | Remote normalization, `deriveProjectGroups()`, config opt-out, API exposure | DONE (2026-07-21) — `src/lib/groups/{types,identity,derive}.ts`, `ungroupedPaths` in `.minder.json`, `ScanResult.groups`; 19 tests. **URL space decided (Risk #5): separate `/group/<slug>` namespace** — every existing `/project/<slug>` keeps its current meaning, and a group/project slug collision is intended rather than a defect. **Risk #4 was a non-issue:** worktree dirs never become `ProjectData` at all — a worktree's `.git` is a file, and `isGitRepo` requires a directory, so they are filtered before slug assignment; a test pins the consequence. **Deviation:** the opt-out is keyed on checkout PATH, not slug, because `resolveProjectSlug` reassigns slugs when `devRoots` is reordered and a slug-keyed list would silently re-merge a split pair. **Blocker found and fixed:** validating against real data showed every UNC/WSL project had `git: undefined` (git's `safe.directory` ownership check, silently swallowed by `runGit`), so the feature's own driver could not group — fixed with a per-directory `-c safe.directory=<cwd>` on Minder's read-only git calls (separators normalized to `/`, or the value never matches the path git reports over UNC). Verified end-to-end: `bamcli` and `micetrocli` now group Windows↔WSL. Review follow-ups on #340: waiver narrowed from a `*` wildcard to the specific directory (Codex), `ssh://`/`git://` remotes now normalized in `scanGit` instead of being dropped (Codex), `groups` omitted rather than sent as `[]` when nothing grouped (Copilot), and `worktreeChecker` given the same flag-position regression test as the scanner (Copilot). |
| P2 | `aggregateGroup()` — four merge rules, derived-metric recomputation | DONE (2026-09-02) — `src/lib/groups/aggregate.ts`, Node-free like the rest of the module; `AggregatableProject` is the `Pick<>` read-set. Repo-borne merges dedupe on stable ids where they exist (insight content hash, board `^i-`/`^e-` surrogates) and on normalized text otherwise (TODO items, manual steps, runbook items); keys are made unique *within* a member by occurrence, so two identical lines in one checkout stay two items. Board title-fallback is container-scoped (inbox vs. inside its merged epic), and `order` is renumbered after dedupe since `line`/`order` are location-bound. Every merged item carries `presentIn` / `completedIn` / `statusIn`; divergences are one entry per file per kind. **Headline rule:** a divergent item shows the *primary* location's value — most recent `lastActivity`, ties to codepoint path order — so output is order-independent (a test reverses the input and asserts deep equality). `ratio()` covers Risk #2; `skippedRootPaths` → `stale`/`partial` covers Risk #7. **Deviations:** (1) environment-borne catalogs are not on `ProjectData`, so the Environments *diff* is P3 work over the catalog API, not part of this function; (2) usage/cost is fetched, not scanned, so P2 exposes `usageKeys` (deduplicated on the `(usageSlug, usageHomeKey)` pair — `C:\dev\foo` and `D:\dev\foo` share a slug) and leaves the summing to P3's fetch layer. Validated against real data: the only live group at the time (`lce911`, two local clones) aggregated correctly — separate dirty counts, primary by recency, two usage keys; the WSL group could not be exercised because the distro was stopped and Minder never wakes it. 27 tests, mutation-checked. |
| P3 | UI: group cards, Locations strip, divergence flags, Environments tab | DONE (2026-09-03) — `/group/<slug>` page (`src/app/group/[slug]/page.tsx`, `src/components/groups/`): Locations strip, activity + repo-fact headline with per-location split, read-only TODOs/Insights/Board/Manual Steps/Ops tabs over the `Aggregated*` shapes with `PresenceChips` divergence flags, Costs via `useGroupUsage` (one `/api/usage` query per `usageKeys` entry, summed by `sumUsageReports`, *approximate* label on non-`exact` keys), Sessions per location, and an Environments tab over the new `GET /api/environments` + `diffEnvironments()`. Location labels from `locationLabels()` (shortest distinguishing path prefix). Demo gains a second `aurora-commerce` checkout. Also carried the two scan-route gaps (`withGroups` in `POST /api/scan` and the MCP scan helper). **Deviations:** (1) *No separate group card.* The plan said "one card per group"; member cards are kept and each gains an `N loc` chip that is the sole entry point. Collapsing members would have moved pins (slug-keyed), the live-session overlay, and the per-member `DevServerControl` onto a group card that duplicates all three, for a dashboard that fetches no usage anyway — so the aggregated view lives on the group page. (2) *No cost on the card* for the same reason; the group page's Costs tab is one click away. (3) *Environments does not read the catalog.* The TODO item said the diff would be built over `/api/agents` + `/api/skills` + `/api/claude-config` keyed by home, but the catalog is bound to `os.homedir()` down to each plugin's `installPath` (a Linux path for a WSL home); a shallow per-home reader (`src/lib/environments/inventory.ts`) answers the narrower question the tab asks. **Resolved 2026-09-04 on `feat/catalog-home-dimension` (#553, pending merge):** `loadCatalog({ home })` walks a configured home by key, `/api/agents?home=` and `/api/skills?home=` serve it, and the Environments tab reads agents/skills from the catalog per home (descriptions, plugin-provided entries); `/api/environments` is reduced to plugins, MCP names and unavailable homes. `/api/claude-config?home=` is still open. (4) The sessions-derived tabs hide in demo mode, matching the project page. **Not built in P3:** a link from a member's `/project/<slug>` page back to its group — shipped 2026-09-04 on `feat/groups-followups` together with per-home session scoping (`SessionSummary.homeKey`, `/api/sessions?home=`). |

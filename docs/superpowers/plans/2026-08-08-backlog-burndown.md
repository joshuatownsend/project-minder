...
2. **#443 — `/usage` throws "circular link" under demo mode** and renders the browser error page instead of the app. The only demo-mode failure that is hard rather than leaky, so it is also the easiest to verify fixed.

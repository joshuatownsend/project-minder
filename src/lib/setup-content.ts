...
export const TRACKED_FILES_NOTE = `These are plain markdown — diffable, mergeable, and reviewable in pull requests. The biggest risk is **stashing** them mid-work: \`git stash pop\` can silently leave the stash in place on conflict, and a later \`git stash drop\` will permanently delete the changes. Commit them rather than stashing — and prefer committing them *with* the feature work they came from, since \`INSIGHTS.md\` in particular records how that change came to look the way it does, which is worth having in the same PR a reviewer is reading. A small \`chore:\` commit is the fallback when there's no related PR to attach them to. Completed work moves into companion \`*.archive.md\` files (e.g. \`TODO.archive.md\`, \`MANUAL_STEPS.archive.md\`) rather than being deleted — commit those too; Project Minder ignores them, so your active counts stay clean.

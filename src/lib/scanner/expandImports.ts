 ...
 *   - Detects cycles via an in-progress recursion stack (NOT a global
 *     visited set) so diamond imports — A→B, A→C, B→D, C→D — include
 *     D's content twice rather than dropping the second branch
...
interface ExpandState {
  ...
  /** Stack of canonical paths currently being expanded — used to detect
   *  true cycles. Distinct from the per-call set so diamond imports
   *  (same file reached from two non-overlapping branches) include twice. */
  stack: Set<string>;
}
...
async function expandFromBuffer(
  ...
  try {
    ...
    for (const line of lines) {
      ...
      // Cycle = the resolved file is already an ancestor in the current
      // branch. Reaching the same file via two independent branches is a
      // diamond, not a cycle, and should expand both times.
      if (state.stack.has(resolved)) {
      ...
      try {
        ...
        const inner = await expandFromBuffer(resolved, innerRaw, importDepth, state);
...
export async function expandImports(
  ...
  const content = await expandFromBuffer(filePath, raw, 0, state);

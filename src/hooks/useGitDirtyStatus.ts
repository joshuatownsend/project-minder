...
export function useGitDirtyStatus() {
  ...
  const fetchStatus = useCallback(async (): Promise<GitStatusResponse | null> => {
    try {
  ...
  // SSE path: the git-status cache pushes an event each time a batch lands, so
  // refetch on push instead of polling. Inert when the flag is off.
  useLiveEvent("git-status.updated", () => {
    void fetchStatus();
  });

...
The gate reads the authoritative `anthropic-ratelimit-unified-*` headers — the same source as the burn HUD, not an estimate from indexed usage. It holds when any window reports a **recognized throttled status** (`throttled`, `rejected`, `blocked`, `exceeded`, `exhausted`), or when utilization reaches **98%** (below 1.0 deliberately: a long task started at 99% hits the wall mid-run and dies with its work unsaved). When more than one window is exhausted, the hold runs to the **latest** reset among them — releasing at the 5-hour reset while the 7-day window is also exhausted would send the queue straight back into the wall.

**Dry-run tasks are not held.** A `dry_run` row never starts Claude and never consumes quota, so while a hold is active the dispatcher claims *only* dry-run rows — filtered in SQL rather than claimed-and-requeued, because priority ordering would otherwise hand back the same held task on every iteration and starve the dry-runs sitting behind it. Real tasks stay `pending` and are picked up on the first tick after the window resets.
...
**Every unclear case fails open.** An *unrecognized* status is not treated as throttled — the quota reader writes the literal string `unknown` whenever the status header is simply absent, so a negative test would have read an ordinary incomplete reading as exhausted and paused the queue. Unfamiliar statuses fall through to the utilization check instead. The gate also does not hold when: quota isn't configured; the reading is older than 30 minutes (`loadQuota()` falls back to a disk cache when its probe fails, so a reading can be arbitrarily old); the reading is future-dated; an exhausted window carries no reset time; the reset has already passed; or the reset is more than 8 days out. A gate that holds on bad data silently stops all background work, which is worse than one visible failed task.

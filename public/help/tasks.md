...
**Dry-run tasks are not held.** A `dry_run` row never starts Claude and never consumes quota, so the hold applies at the point it means something — immediately before a real spawn — rather than blocking the claim loop. A held real task is requeued to `pending` rather than failed: it never started, so it belongs back in the queue for the tick after the window resets.
